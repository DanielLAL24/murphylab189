...
if (w(1) == 1)
        ...
        feat_vector_ori = [feat_vector_ori,feat_extract(im4,im5,'haralick',imgcent_coordinate)];
...
if (w(2) == 1)
        ...
        feat_vector_ori = [feat_vector_ori,feat_extract(im4,im5,'histpropwithcent',imgcent_coordinate)];
        idxes{2}(2) = size(feat_vector_ori,2);
end

if (w(3) == 1)
        idxes{3}(1) = size(feat_vector_ori,2)+1;
        feat_vector_ori = [feat_vector_ori,feat_extract(im4,im5,'harkd2',imgcent_coordinate)];
        idxes{3}(2) = size(feat_vector_ori,2);
end

if (w(4) == 1)
        idxes{4}(1) = size(feat_vector_ori,2)+1;
        feat_vector_ori = [feat_vector_ori,feat_extract(im4,im5,'harkd4',imgcent_coordinate)];
...
if (w(6) == 1)
        ...
        feat_vector_ori = [feat_vector_ori,feat_extract(im4,im5,'radIntensity',imgcent_coordinate)];
        idxes{6}(2) = size(feat_vector_ori,2);
end

if (w(7) == 1)
        idxes{7}(1) = size(feat_vector_ori,2)+1;
        feat_vector_ori  = [feat_vector_ori,feat_extract(im4,im5,'edge',imgcent_coordinate)];
