function mainScript10_hela_acute(n_inputArray,rand_start,batchno,cellnum,subfolder,thr)

% Loading Dboth from cell1 that has been semi automatically segmented
load(['proc_' num2str(thr) '/cell_' num2str(cellnum) '/final_info.mat'],'Dbothfin','imgcent_coordinate');
XYZres = 0.2;

folderName = ['outputs_' num2str(subfolder) '/images/thr_' num2str(thr) '/cell_' num2str(cellnum) '/batch_' num2str(batchno) '/new_n_' num2str(n_inputArray)];
  ...
  for mu_len = mulen_inputArray
    for coeffvar = coeff_var_Array
      ...
      for colli_min_number = colli_inputArray
		...
		if subfolder<=2
                K = colli_generator_acute(n,mu_len,sigma_len,colli_min_number,Dbothfin,imgcent_coordinate,XYZres,0.4,0.2,rand_seed,cellnum,subfolder,batchno,thr);
		else
		  [K,tcheck] = colli_generator_acute_jl2(n,mu_len,sigma_len,colli_min_number,Dbothfin,imgcent_coordinate,XYZres,0.4,0.2,rand_seed,cellnum,subfolder,batchno,thr);
		end
             end  % end of myfile
	      try
             fclose(fid);
