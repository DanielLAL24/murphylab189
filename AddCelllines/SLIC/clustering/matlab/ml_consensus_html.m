function [g c]= ml_consensus_html(feat, name,urls, htmlfile, html_URL, left,...
			       width, distance, method, penc, niter)

%function [g c]= ml_consensus_html(feat, name,name2,urls, htmlfile, html_URL, left,...
%			       width, distance, method, penc, niter)
...
%       representing a clone, each element itself is a cell array where elements
...
if 1==1
  ...
  for m = 1 : length(feat)
	     ...
	     labels{length(features)} = name2{m};  %name2
  ...
  %keyboard;
if 1==2 %Because the figure of tree (consensus tree or hierarchical tree) has already been created.
...
%  eval('label = ml_drawconsensus(links, labels,gene, 8, ma);', ...
%       'label = ml_drawconsensus(links(1:end-1), labels, gene, 8, ma);');
  eval('label = ml_drawconsensus(links, gene, 8, ma);', ...
       'label = ml_drawconsensus(links(1:end-1), gene, 8, ma);');
  saveas(gcf, [prefix '.jpg'], 'jpeg');
end

load links_draw label
