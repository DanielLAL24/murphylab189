function [G,tcheck,imXYZ,mtXYZ,randlengths] = colli_generator_acute_jl2(n,mu_len,sigma_len,colli_min_number,Dboth,imgcent_coordinate,XYZres,centroradius,stepsize,rand_seed,cellnum,setno,fieldno,subfolder,batchno,thr,colli_thre,angle_thre,checkback,nosave,nomcheck)
...
for j = 1:n
  if randlengths(j) ~= 0
	...
	while 1
              counter1 = counter1 + 1;
              if counter1>=50 return; end  %% jl++, 02-27-2012
		...
		if  ((tempmt2(1) <= size(Dboth,1)) && (tempmt2(2) <= size(Dboth,2)) && (tempmt2(1) >= 1) ... 
			...
			if (Dboth(tempmt2(1),tempmt2(2),tempmt2(3)) == 0)
				...
				if randlengths(j) == 1
					...
					counter2 = counter2 + 1;
					if counter2>=50 return; end  %% jl++, 02-27-2012
...
if nosave==0
save(['outputs_' num2str(subfolder) '/images/set_' num2str(setno) '/fieldno_' num2str(fieldno) '/thr_' num2str(thr) '/Image_' num2str(cellnum) '/batch_' num2str(batchno) '/new_n_' num2str(n) '/new_sim_n_' num2str(n) '_mulen_' num2str(mu_len) '_siglen_' num2str(sigma_len) '_colli_' num2str(colli_min_number) '.mat'], 'imgcent_coordinate', 'imXYZ', 'mtXYZ', 'G', 'n','randlengths','colli_thre','angle_thre','checkback');

delete(['outputs_' num2str(subfolder) '/images/set_' num2str(setno) '/fieldno_' num2str(fieldno) '/thr_' num2str(thr) '/Image_' num2str(cellnum) '/batch_' num2str(batchno) '/new_n_' num2str(n) '/new_sim_n_' num2str(n) '_mulen_' num2str(mu_len) '_siglen_' num2str(sigma_len) '_colli_' num2str(colli_min_number) '-tcheck.mat']);  %%
end
